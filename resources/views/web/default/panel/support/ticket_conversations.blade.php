                ...
                <form action="/panel/support/tickets" method="get">
                    <div class="row">
                                        ...
                                        <select name="department" id="departments" class="form-control">
                                            ...
                                            @foreach ($departments as $department)
                                                <option value="{{ $department->id }}"
                                                    @if (request()->get('department') == $department->id) selected @endif>
                                                    {{ $department->title }}
                                                </option>
                                            @endforeach
                                        </select>
                        ...
                        <div class="col-12 col-lg-2 d-flex align-items-center justify-content-end">
                            <button type="submit"
                                class="btn btn-sm font-14 btn-primary w-100 mt-2">{{ trans('public.show_results') }}</button>
                        </div>
                    </div>
                </form>
            </div>
        ...
        @if (!empty($supports) and !$supports->isEmpty())
            ...
            <div class="bg-white shadow rounded-sm py-10 py-lg-25 px-15 px-lg-30 mt-25">
                <div class="row">
                    ...
                    @if (!empty($selectSupport))
                        <div class="col-12 col-lg-12 border-left border-gray300">
                            <div class="conversation-box p-15 d-flex align-items-center justify-content-between">
                                <div>
                                    <span
                                        class="font-weight-500 font-14 text-dark-blue d-block">{{ $selectSupport->title }}</span>
                                    {{-- <span class="font-12 text-gray d-block mt-5">{{ trans('public.created') }}:
                                        {{ dateTimeFormat($support->created_at, 'j M Y | H:i') }}</span> --}}

                                    @if (!empty($selectSupport->webinar))
                                        <span class="font-12 text-gray d-block mt-5">{{ trans('webinars.webinar') }}:
                                            {{ $selectSupport->webinar->title }}</span>
                                    @endif
                                </div>

                                @if ($selectSupport->status != 'close')
                                    <a href="/panel/support/{{ $selectSupport->id }}/close"
                                        class="btn btn-primary btn-sm">{{ trans('panel.close_request') }}</a>
                                @endif
                            </div>
                            ...
                            <div id="conversationsCard" class="pt-15 conversations-card">
                                ...
                                @if (!empty($selectSupport->conversations) and !$selectSupport->conversations->isEmpty())
                                    @foreach ($selectSupport->conversations as $conversations)
                                        @isset($conversations->message)
                                            <div
                                                class="rounded-sm mt-15 border panel-shadow p-15 @if (isset($conversations->sender_id)) my-message  @else support-message @endif">
                                                <div
                                                    class="d-flex align-items-center justify-content-between pb-20 border-bottom border-gray300">
                                                    <div class="user-inline-avatar d-flex align-items-center">
                                                        <div class="avatar bg-gray200">
                                                            <img src="{{ !empty($conversations->supporter) ? $conversations->supporter->getAvatar() : $conversations->sender->getAvatar() }}"
                                                                class="img-cover" alt="">
                                                        </div>
                                                        <div class="ml-10">
                                                            <span
                                                                class="d-block font-14 font-weight-500">{{ !empty($conversations->supporter) ? $conversations->supporter->full_name : $conversations->sender->full_name }}</span>

                                                            <span
                                                                class="mt-1 font-12  d-block">{{ !empty($conversations->supporter) ? $conversations->supporter_role()->caption : 'دانشجو' }}</span>
                                                        </div>
                                                    </div>

                                                    <div class="d-flex flex-column align-items-end">
                                                        <span
                                                            class="font-12 ">{{ dateTimeFormat($conversations->created_at, 'j M Y | H:i') }}</span>

                                                        @if (!empty($conversations->attach))
                                                            <a href="{{ url($conversations->attach) }}" target="_blank"
                                                                class="font-12 mt-10 text-danger"><i data-feather="paperclip"
                                                                    height="14"></i> {{ trans('panel.attach') }}</a>
                                                        @endif
                                                    </div>
                                                </div>
                                                <p class=" mt-15 font-weight-500 font-14">
                            ...
                            <div class="conversation-box mt-30 py-10 px-15">
                                <h3 class="font-14 text-dark-blue font-weight-bold">
                                    {{ trans('panel.reply_to_the_conversation') }}</h3>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <input type="text" name="attach" id="attach"
                                                    value="{{ old('attach') }}" class="form-control" />
                                            </div>
                                        </div>
                        ...
                        <div id="conversationsList" class="col-12 col-lg-6 conversations-list">
                            <div class="table-responsive">
                                <table class="table table-md">
                                    <tr>
                                        <th class="text-left font-14 text-gray font-weight-500">
                                            {{ trans('navbar.title') }}
                                        </th>
                                        <th class="text-center font-14 text-gray font-weight-500">
                                            {{ trans('public.updated_at') }}</th>
                                        <th class="text-center font-14 text-gray font-weight-500">
                                            {{ trans('panel.department') }}</th>
                                        <th class="text-center font-14 text-gray font-weight-500">
                                            {{ trans('public.status') }}
                                        </th>
                                    </tr>
                                        ...
                                        @foreach ($supports as $support)
                                            <tr class="@if (!empty($selectSupport) and $selectSupport->id == $support->id) selected-row @endif">
                                                <td class="text-left">
                                                    ...
                                                    <div class="user-inline-avatar d-flex align-items-center">
                                                        <div class="avatar bg-gray200">
                                                            <img src="/assets/default/img/support.png" class="img-cover"
                                                                alt="">
                                                        </div>
                                                        <div class="ml-10">
                                                            <span
                                                ...
                                                <td class="text-center align-middle">
                                                    <span
                                                        class="font-weight-500 text-dark-blue font-14 text-gray d-block">{{ (!empty($support->conversations) and count($support->conversations)) ? dateTimeFormat($support->conversations->first()->created_at, 'j M Y | H:i') : dateTimeFormat($support->created_at, 'j M Y | H:i') }}</span>
                                                </td>

                                                <td class="text-center align-middle">
                                                    <span
                                                        class="font-weight-500 text-dark-blue font-14 d-block">{{ $support->department->title }}</span>
                                                </td>
